...
const CategoryAcordion = () => {
  return (
    <Collapsible className="border dark:border-primary rounded-md shadow-sm ">
      <CollapsibleTrigger className="flex rounded-tl-md w-full md:max-w-[400px] items-center justify-between px-4 py-3 mb-2 bg-gray-100 dark:bg-gray-800 border-b border-r md:rounded-br-3xl">
      ...
      <CollapsibleContent className="px-4 py-3 ransition-all duration-300 ease-in-out">
        {mockProducts.map((item) => (
          <ProductAccordion key={item.name} data={item} />
        ))}
      </CollapsibleContent>
    </Collapsible>
  );
          ...
          <div className="flex flex-col space-y-2 mt-2">
            ...
            <div>
              ...
              {mockSubProducts.map((item) => (
                <SubProductAcordion key={item.name} data={item} />
              ))}
            </div>
          </div>
        </div>
