...
import { updatePackage } from "@/server-actions/product";
...
const EditPackageForm = ({ data }: { data: Package }) => {
	...
	const form = useForm<CreatePackage>({
		...
		defaultValues: {
			...
			//@ts-ignore
			additionalFeature: data.additionalFeature,
		},
	});
	...
	const onSubmit = (values: CreatePackage) => {
		...
		startTransition(() => {
			updatePackage(data.id, values).then((data) => {
				setError(data.error);
	...
	return (
		<DataFormWrapper title="Update Package">
			<Form {...form}>
				<form
